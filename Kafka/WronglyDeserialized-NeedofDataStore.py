...
async def consume(topic_name):
    ...
    while True:
        ...
        if message is None:
            ...
            # Load the value as JSON and then create a Purchase object. The web team has told
            #       us to expect the keys "username", "currency", and "amount". #wrongly deserialized
            clickevent_json = json.loads(message.value())
            try:
                print(
                    ClickEvent(
                        email=clickevent_json["username"],
                        timestamp=clickevent_json["currency"],
                        uri=clickevent_json["amount"],
                    )
                )

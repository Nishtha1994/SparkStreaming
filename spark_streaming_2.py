...
def run_spark_streaming():
    ...
    """
    #  based on the df.show, build schema
    jsonSchema = StructType([StructField("status", StringType(), True),
                             StructField("timestamp", TimestampType(), True)])

    #  create streamning input dataframe that reads the same dataset, with jsonSchema you created
    streamingInputDF = (
        spark
            .readStream

...
def join_exercise():
    ...
    """
    Watermarks can be late/out-of-order in event-time by at most 20 and 30 seconds, respectively.
    """
    #  use watermark of 10 seconds
    left_with_watermark = left \
        .selectExpr("row_id AS left_row_id", "left_timestamp") \
        .withWatermark("left_timestamp", "10 seconds ")

    #  use watermark of 20 seconds
    right_with_watermark = right \
        .selectExpr("row_id AS right_row_id", "right_timestamp") \
        .withWatermark("right_timestamp", "20 seconds")
    
    """
    Right can occur within a time range of 0 seconds to 1 minutes after the corresponding left.
    """
